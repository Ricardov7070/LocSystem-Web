...
export function DataTableFilterList<TData>({
  ...
  function updateFilter({
    ...
    updateFunction((prevFilters: FilterCondition<TData>[]) => {
      const updatedFilters = prevFilters.map((filter: FilterCondition<TData>, i: number) => {
        if (i === index) {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          <Filter className="mr-2 size-3.5" aria-hidden="true" />
          Filters
          {filters.length > 0 && (
            <Badge
              variant="secondary"
